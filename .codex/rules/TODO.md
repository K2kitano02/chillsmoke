...
# ISSUE-113 +1記録操作をTurbo Stream化する

## なぜ必要か

- ダッシュボードの `+1` は喫煙記録で最も頻繁に使う操作であり、押すたびに画面全体が再読み込みされると操作感が重くなるため
- 保存処理は既存のサーバー側ロジックを維持しつつ、更新が必要な表示だけを差し替えることで、体験を改善できるため
- 面接でも「頻繁に使う操作を部分更新し、ただし加算処理はDBロックで守った」と説明しやすい改善になるため

## 必要なこと(簡易的に)

- 現在の `+1` 加算処理、`with_lock`、validation、flash表示の流れを確認する
- ダッシュボード上で `+1` 後に更新されるべき表示を洗い出す
- 通常HTMLリクエストでも動く fallback を残す

## 仕様（確定）

- JavaScriptの `fetch` 直書きではなく、Rails標準の Turbo Stream で実装する
- `+1` の保存処理、validation、`with_lock` は既存のcontroller/model/service側に残す
- 成功時は保存後の正しいDB状態をもとに、関連partialをTurbo Streamで差し替える
- フロント側で本数だけを楽観的に増やして表示しない
- 失敗時は画面全体を500にせず、flashまたはエラー表示をTurbo Streamで更新する

## やること(コードレベル)

- **変更点（ファイル）**: `app/controllers/*` / `app/views/dashboard/*` / `app/views/shared/*` / 必要に応じて `app/views/*/*.turbo_stream.erb` / `test/*`
<!-- 実装クリア: ISSUE-113
- [x] `+1` を処理しているcontroller actionと現在のredirect先を確認する
- [x] `+1` 後に更新する表示をpartial化または既存partialに整理する
  - [x] 今日の喫煙本数
  - [x] 今日の見込み節約額
  - [x] 使用可能金額
  - [x] headerの使用可能金額・継続日数
  - [x] flash
- [x] `respond_to` または `.turbo_stream.erb` でTurbo Stream応答を追加する
- [x] HTMLリクエスト時は従来通りredirectされるようにする
- [x] 連打時もDB上の本数が破綻しないことをテストで確認する
- [x] Docker経由でテストを実行する
- [x] Playwright/MCPで `+1` 押下後に画面全体が再読み込みされず、必要表示だけ更新されることを確認する
-->

## 変更してはいけないこと

<!-- 実装クリア: ISSUE-113
- [x] `with_lock` やDB制約など、加算漏れ防止のサーバー側防御を弱めない
- [x] GETアクセスだけでログを作成しない
- [x] 今日分の見込みと昨日までの確定分の扱いを混ぜない
- [x] 認証、設定、購入処理など無関係な仕様を変更しない
-->

## ゴール

<!-- 実装クリア: ISSUE-113
- [x] `+1` 押下後、画面全体を再読み込みせずに本数が更新される
- [x] 使用可能金額、今日の見込み、header表示が保存後の正しい値で更新される
- [x] 連打しても加算漏れや二重反映が起きない
- [x] 通常リロード後も表示が正しい
- [x] Docker経由のテストが通る
-->
...
# ISSUE-114 ウィッシュリスト購入操作をTurbo Stream化する

## なぜ必要か

- 購入ボタン押下後に画面全体を再読み込みしなくても、更新対象は使用可能金額、対象アイテム、購入履歴、flashに限定できるため
- 購入後の状態変化を即座に画面へ反映すると、節約額を欲しいものへ変換する体験が分かりやすくなるため
- ただし購入処理は残高チェック、transaction、ロックが重要なので、サーバー側の防御を維持したまま部分更新する必要があるため

## 必要なこと(簡易的に)

- wishlist購入処理のcontroller、model、service、transaction、ロックの流れを確認する
- 購入後に更新する画面要素を整理する
- 残高不足や二重購入時のエラー表示を確認する

## 仕様（確定）

- JavaScriptの `fetch` 直書きではなく、Turbo Streamで実装する
- 購入可否判定、残高チェック、transaction、ロックは既存のサーバー側処理を維持する
- 成功時は対象wishlist行、使用可能金額、購入履歴、flashを更新する
- 失敗時は対象画面に日本語のエラーを表示し、500画面にしない
- 通常HTMLリクエストでも従来通り動くfallbackを残す

## やること(コードレベル)

- **変更点（ファイル）**: `app/controllers/*purchases*` / `app/controllers/*wishlists*` / `app/views/wishlists/*` / `app/views/shared/*` / 必要に応じて `app/views/*/*.turbo_stream.erb` / `test/*`
<!-- 実装クリア: ISSUE-114
- [x] 購入ボタンの現在のリクエスト先とredirect先を確認する
- [x] 購入後に更新するpartialを整理する
  - [x] headerの使用可能金額
  - [x] wishlistの対象行または対象カード
  - [x] 購入履歴
  - [x] flash
- [x] 成功時のTurbo Stream応答を追加する
- [x] 残高不足、購入済み、同時実行時の失敗応答をTurbo Streamで表示する
- [x] HTMLリクエスト時は従来通りredirectされるようにする
- [x] Docker経由で購入処理のテストを実行する
- [x] Playwright/MCPで購入成功、残高不足、スマホ幅の表示を確認する
-->

## 変更してはいけないこと

<!-- 実装クリア: ISSUE-114
- [x] transactionやロックを外さない
- [x] フロント側だけで購入済み表示にしない
- [x] 残高計算の仕様を変更しない
- [x] 購入履歴を削除・再作成するような破壊的な更新にしない
-->

## ゴール

<!-- 実装クリア: ISSUE-114
- [x] 購入ボタン押下後、画面全体を再読み込みせずに対象表示が更新される
- [x] 使用可能金額、wishlist状態、購入履歴が保存後の正しい状態になる
- [x] 残高不足や二重購入時に日本語エラーが表示される
- [x] Docker経由のテストが通る
-->

---

# ISSUE-115 スケジュール有効/無効トグルをTurbo Stream化する

## なぜ必要か

- スケジュール一覧では有効/無効の切り替えだけを行うため、画面全体を再読み込みする必要が薄い
- 対象行だけを更新できると、スマホでも操作結果が分かりやすくなるため
- トグルUIの見た目とDB上の有効状態を同期させる必要があるため

## 必要なこと(簡易的に)

- スケジュールの有効/無効を切り替えるcontroller actionを確認する
- トグル表示、状態バッジ、flashの更新箇所を確認する
- 通常HTMLリクエストでも動くfallbackを残す

## 仕様（確定）

- JavaScriptの `fetch` 直書きではなく、Turbo Streamで実装する
- 切り替え処理はサーバー側で保存し、保存後の状態で対象行を差し替える
- 成功時は対象スケジュール行、トグル状態、状態バッジ、flashを更新する
- 失敗時はflashまたは対象行にエラーを表示する

## やること(コードレベル)

- **変更点（ファイル）**: `app/controllers/user_schedules_controller.rb` / `app/views/user_schedules/*` / 必要に応じて `app/views/*/*.turbo_stream.erb` / `test/*`
<!-- 実装クリア: ISSUE-115
- [x] 有効/無効切り替えactionとルーティングを確認する
- [x] スケジュール1行分を差し替えやすいpartialに整理する
- [x] `turbo_frame_tag` または `turbo_stream.replace` の対象IDを決める
- [x] 成功時のTurbo Stream応答を追加する
- [x] 失敗時の日本語エラー表示を追加する
- [x] HTMLリクエスト時は従来通りredirectされるようにする
- [x] Docker経由でスケジュール更新テストを実行する
- [x] Playwright/MCPでPC幅・スマホ幅の切り替え表示を確認する
-->

## 変更してはいけないこと

<!-- 実装クリア: ISSUE-115
- [x] 有効/無効以外のスケジュール属性を勝手に変更しない
- [x] 他ユーザーのスケジュールを更新できるようにしない
- [x] トグル表示だけ変えてDB保存を省略しない
-->

## ゴール

<!-- 実装クリア: ISSUE-115
- [x] 有効/無効トグル押下後、画面全体を再読み込みせずに対象行が更新される
- [x] トグル状態とDB上の有効状態が一致する
- [x] スマホ幅で状態ラベルやトグル文字が潰れない
- [x] Docker経由のテストが通る
-->

---

# ISSUE-116 スケジュール反映操作をTurbo Stream化する

## なぜ必要か

- スケジュール反映は、反映済み状態、今日の本数、使用可能金額、flashなど限られた表示だけが変わるため
- 画面全体リロードを避けることで、反映できたかどうかをその場で確認しやすくなるため
- 反映処理は重複反映防止が重要なので、サーバー側の既存ロジックを維持したまま部分更新する必要があるため

## 必要なこと(簡易的に)

- スケジュール反映処理のcontroller、service、重複防止条件を確認する
- 反映成功時と「未反映なし」時に更新する表示を整理する
- 既存のテストで反映済みスケジュールが二重加算されないことを確認する

## 仕様（確定）

- JavaScriptの `fetch` 直書きではなく、Turbo Streamで実装する
- 反映処理、重複防止、日付判定、DB保存は既存のサーバー側ロジックを維持する
- 成功時は反映済み状態、今日の本数、使用可能金額、今日の見込み、flashを更新する
- 反映対象がない場合も画面全体を再読み込みせず、flashで「未反映なし」を表示する
- 通常HTMLリクエストでも従来通り動くfallbackを残す

## やること(コードレベル)

- **変更点（ファイル）**: `app/controllers/schedule_reflections_controller.rb` / `app/views/user_schedules/*` / `app/views/dashboard/*` / `app/views/shared/*` / 必要に応じて `app/views/*/*.turbo_stream.erb` / `test/*`
<!-- 実装クリア: ISSUE-116
- [ ] 反映ボタンの現在のリクエスト先とredirect先を確認する
- [ ] 反映後に更新するpartialを整理する
  - [ ] スケジュール一覧の反映状態
  - [ ] 今日の喫煙本数
  - [ ] 使用可能金額
  - [ ] 今日の見込み節約額
  - [ ] flash
- [ ] 反映成功時のTurbo Stream応答を追加する
- [ ] 反映対象なしの場合のTurbo Stream応答を追加する
- [ ] HTMLリクエスト時は従来通りredirectされるようにする
- [ ] 重複反映されないことをDocker経由のテストで確認する
- [ ] Playwright/MCPで1回目反映、2回目未反映なし、console errorなしを確認する
      -->
...
- [ ] 重複反映防止の条件を弱めない
- [ ] 無効スケジュールを反映対象にしない
- [ ] GETアクセスだけで反映処理を実行しない
- [ ] 今日分の見込みと昨日までの確定分の扱いを混ぜない
      -->

## ゴール

<!-- 実装クリア: ISSUE-116
- [ ] 反映ボタン押下後、画面全体を再読み込みせずに対象表示が更新される
- [ ] 有効スケジュールだけが反映される
- [ ] 2回目押下時は二重加算されず、「未反映なし」が表示される
- [ ] Docker経由のテストが通る
      -->

---

# ISSUE-118 BulletでN+1問題を検出・解消する

## なぜ必要か

- 一覧画面や共通ヘッダーで、表示件数に比例してSQLが増えるN+1問題が発生していないか確認するため
- Bulletを利用して、関連データの取得漏れだけでなく、不要なeager loadingも検出できる開発環境を整えるため
- 推測だけでリファクタリングせず、修正前後のクエリを比較し、パフォーマンス改善の根拠を残すため

## 必要なこと(簡易的に)

- 現在の主要な一覧・詳細画面と、共通ヘッダーで実行されるクエリを確認する
- 複数件の喫煙ログ、スケジュール、ウィッシュリスト、購入履歴がある検証データを用意する
- Bulletは開発環境だけで有効化し、本番環境の動作や利用者データに影響を与えない
- N+1が検出されなかった画面も含め、確認対象と結果をIssueまたはPRへ記録する

## 仕様（確定）

- Bulletを導入し、development環境でN+1、unused eager loading、counter cache候補を確認できるようにする
- Bulletの通知は開発時にブラウザまたはログで確認できるようにし、アプリの通常操作を妨げない設定にする
- 次の主要GET画面を確認対象にする
  - ダッシュボード
  - 喫煙記録カレンダー
  - 喫煙スケジュール一覧
  - ウィッシュリスト一覧
  - ウィッシュリスト詳細・購入履歴
- 検出されたN+1は、ユーザーデータを必ず`current_user`配下から取得する既存方針を維持したまま解消する
- `includes`、`preload`、`eager_load`などは、実際に関連データを参照する箇所だけへ追加する
- N+1が検出されない画面には、不要な先読みを追加しない
- 修正前後で、対象画面のクエリ数またはBulletの検出結果を比較できる記録を残す
...
<!-- 実装クリア: ISSUE-118
- [ ] N+1対策を目的に、金額、snapshot、継続日数、購入、スケジュール反映の計算仕様を変更しない
- [ ] GETアクセスだけで`user_smoking_logs`を作成しない
- [ ] `current_user`によるユーザーデータのスコープを外さない
- [ ] N+1が確認できていない箇所へ推測で先読みを追加しない
- [ ] Bulletをproduction環境で有効化しない
- [ ] N+1対策と無関係な画面変更やリファクタリングを混ぜない
-->
...
# ISSUE-119 喫煙本数と節約額の推移をグラフで表示する

## なぜ必要か

- レビューで、日ごとの記録は確認できるものの、長期的に減煙できているか把握しづらいという意見をもらったため
- 喫煙本数と節約額の推移を可視化し、ユーザーが減煙による変化と継続の成果を実感しやすくするため
- レビューで見つかったユーザー体験上の課題を、仕様・実装・検証へ落とし込む開発プロセスを残すため

## 必要なこと(簡易的に)

- 現在の喫煙ログ、snapshot、節約額計算の責務を確認する
- Rails 7.2、esbuild、Stimulus構成で利用できるグラフライブラリを比較する
- グラフ用データの作成と描画処理を分離し、controllerやviewへ計算処理を集中させない
- 複数件、未記録日、0本記録、目標未達、鬼モードを含むデータで確認する

## 仕様（確定）

- ログインユーザー専用の「減煙レポート」画面を追加する
- 1ページ内に「喫煙本数」と「節約額」の2種類のグラフを配置する
- タブ操作により、ページ全体を再読み込みせずグラフを切り替える
- 初期表示は「喫煙本数」とする
- 表示期間は当日を含む直近30日間とし、期間切り替えは本Issueに含めない
- 日付ラベルは30日分を連続して用意し、未記録日を飛ばさない

### 喫煙本数グラフ

- 日ごとの喫煙本数を折れ線で表示する
- 日ごとの目標本数を比較用の系列として表示する
- 記録がある日の目標本数は`target_daily_cigarette_count_snapshot`を使う
- 当日はログが存在する場合だけ実績とsnapshot目標を表示する
- 未記録日は`0本`にせず、実績・目標ともに欠損値として線を途切れさせる

### 節約額グラフ

- 日ごとの節約額を棒グラフで表示する
- 昨日までの記録済みログは、各ログのsnapshotに基づく`saved_yen`を使う
- 未記録日は`0円`にせず欠損値として扱う
- 当日分は確定額としてグラフへ含めず、既存仕様どおり「今日の節約見込み」としてグラフ外へ別表示する
- 鬼モードで目標を超えた日の節約額は、既存計算どおり0円とする

### タブ・表示

- 選択中のタブが見た目と`aria-selected`等で判別できる
- キーボード操作でもタブを切り替えられる
- 390px程度のスマホ幅でも、グラフや日付ラベルが画面外へはみ出さない
- データが1件もない場合は、グラフを壊さず記録を促す日本語メッセージを表示する
- ダッシュボードまたは共通ナビゲーションから減煙レポートへ移動できる
...
- [x] グラフライブラリを比較し、選定理由をPRへ記録する
  ...
  - [x] CDNへ依存せず、既存のアセット管理へ組み込む
- [x] 減煙レポート画面のGETルートとcontroller actionを追加する
- [x] `current_user.user_smoking_logs`から直近30日分だけを取得する
- [x] 30日分の日付、喫煙本数、snapshot目標、日別節約額をグラフ用データへ変換する
- [x] 未記録日を`nil`相当の欠損値として出力する
- [x] 今日の節約見込みを既存計算から取得し、グラフ外へ表示する
- [x] 喫煙本数の折れ線グラフを描画する
- [x] 節約額の棒グラフを描画する
- [x] Stimulus等でタブ切り替えを実装する
- [x] ダッシュボードまたは共通ナビゲーションへ導線を追加する
- [x] データなし表示を追加する
- [x] グラフ用データのテストを追加する
  - [x] 直近30日の日付が古い順に並ぶ
  - [x] 30日より前のログを含めない
  - [x] 他ユーザーのログを含めない
  - [x] 未記録日が0ではなく欠損値になる
  - [x] 0本として記録した日は0として表示される
  - [x] 目標本数は現在設定ではなく各ログのsnapshotを使う
  - [x] 昨日までの日別節約額は各ログのsnapshotと鬼モードを反映する
  - [x] 今日の節約見込みを確定額の系列へ含めない
- [x] レポート画面のGETで`user_smoking_logs`が作成されないことをテストする
...
- グラフライブラリは`Chart.js 4.5.1`を採用した
  - 折れ線・棒・複数系列・`null`による欠損・レスポンシブ表示を単体で扱える
  - Chartkick等の追加ラッパーを挟まず、既存のesbuildとStimulusへ直接組み込める
  - `package.json`と`yarn.lock`で管理し、CDNへ依存しない
- 直近7日の日別カードと色分けを追加し、目標以内・目標超過・未記録をホバーなしで確認できるようにした
- PlaywrightでPC幅と390px幅を確認し、ページ横幅がviewportを超えないこと、タブとキーボード操作、空状態、console errorなしを確認した
- Bulletログに`GET /report`由来のN+1・unused eager loading警告がないことを確認した
- `docker compose exec web bin/rails test`: 239 runs / 968 assertions / 0 failures / 0 errors
- `docker compose exec web bundle exec rubocop`: 98 files / no offenses
- `docker compose exec web bin/brakeman --no-pager`: Security Warnings 0
...
- [ ] GETアクセスだけで`user_smoking_logs`を作成しない
- [ ] `current_user`のスコープを外して他ユーザーのログを取得しない
...
# ISSUE-120 記録平均から30日間の減煙・節約ペースを表示する

## なぜ必要か

- グラフだけでは、現在の記録が目標に対して良いペースなのかユーザー自身で読み解く必要があるため
- 直近の平均と30日間の目安を文章で示し、減煙を続けた場合の成果を具体的にイメージしやすくするため

## 必要なこと(簡易的に)

- ISSUE-119の減煙レポート画面とグラフ用データが完成していること
- 平均や目安を断定的な予測として表示せず、計算期間と前提を明示すること
- 未記録日がある場合の扱いを、0として補完せず明確にすること

## 仕様（確定）

- 対象期間は昨日から遡る直近7日間とし、当日は含めない
- 7日間すべてに喫煙ログがある場合だけ平均とペースの目安を表示する
- 未記録日がある場合は計算せず、表示に必要な残り記録日数を案内する
- 喫煙本数の平均は、7日分の`smoking_count`合計を7で割り、小数を使わず整数へ切り上げて「1日平均 約N本」と表示する（例: `23 / 7 = 3.28...`は`約4本`）
- 目標との差は小数の平均差へ丸めず、7日分の`smoking_count`合計と、各ログの`target_daily_cigarette_count_snapshot`合計を比較する
  - 実績合計が少ない場合は「7日間では目標よりN本少なく記録できました」と表示する
  - 実績合計が多い場合は「7日間では目標よりN本多い記録でした」と表示する
  - 実績合計と目標合計が同じ場合は「7日間では目標どおりの記録でした」と表示する
- 節約額の1日平均は、7日分の`saved_yen`合計を7で割り、1円未満を切り捨てる
- 30日間の節約ペースは、丸める前の7日分の節約額合計を基準に`合計 ÷ 7 × 30`で算出し、最後に1円未満を切り捨てる
- 「現在のペースを30日続けた場合の目安」であり、将来の成果を保証しない文言にする
- 健康効果、禁煙達成日、医療上の効果は予測しない

## やること(コードレベル)

- **変更候補（ファイル）**: `app/services/*`または`app/queries/*` / `app/controllers/*reports*` / `app/views/*reports*` / `test/*` / `.codex/rules/TODO.md` / 必要に応じて`README.md`
<!-- 実装クリア: ISSUE-120
- [x] 昨日までの直近7日間のログを`current_user`配下から取得する
- [x] 7日分の記録が揃っているか判定する
- [x] 平均喫煙本数を整数へ切り上げ、7日分の実績合計とsnapshot目標合計の差を計算する
- [x] 平均日別節約額と30日間の節約ペースを計算する
- [x] 目標以内・目標超過・目標同数それぞれの日本語メッセージを表示する
- [x] 記録不足時に残り日数を表示する
- [x] 喫煙本数平均の切り上げ、節約額の切り捨て、30日ペースを途中で丸めないこと、鬼モード、未記録日、他ユーザー除外のテストを追加する
- [x] Docker経由でテスト、RuboCop、Brakemanを実行する
- [x] Playwright/MCPでデータ充足時・不足時・スマホ幅・console errorなしを確認する
-->

## 検証結果

- `Reports::PaceData`で昨日までの直近7日だけを取得し、7日分が揃った場合だけ平均とペースを返すことを確認した
- 平均喫煙本数は`fdiv(7).ceil`で整数へ切り上げ、節約額は`div(7)`で1円未満を切り捨てることを確認した
- 30日間の節約ペースは`(7日分の節約額合計 * 30).div(7)`で、1日平均を途中で丸めず算出することを確認した
- Playwrightで7日充足時と5日記録時を確認し、390px幅で横スクロールがなく、console error・warningが0件であることを確認した
- `docker compose exec web bin/rails test`: 249 runs / 1018 assertions / 0 failures / 0 errors
- `docker compose exec web bundle exec rubocop`: 100 files / no offenses
- `docker compose exec web bin/brakeman --no-pager`: Security Warnings 0

## 変更してはいけないこと

<!-- 実装クリア: ISSUE-120
- [ ] 未記録日を0本または0円として平均へ含めない
- [ ] 今日の見込みを平均や30日間の目安へ含めない
- [ ] 過去ログを現在設定で再計算しない
- [ ] GETアクセスだけでログを作成しない
- [ ] 医療効果や禁煙達成時期を断定しない
- [ ] ISSUE-119のグラフ描画仕様を同時に大きく変更しない
-->

## ゴール

<!-- 実装クリア: ISSUE-120
- [x] 7日分の記録が揃うと、整数へ切り上げた平均喫煙本数と、7日合計での目標との差を確認できる
- [x] 現在の平均節約額から30日間の節約ペースを確認できる
- [x] 記録不足時は誤った平均を出さず、必要な残り記録日数が表示される
