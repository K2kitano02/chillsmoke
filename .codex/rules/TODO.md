...
- 喫煙本数の平均は、7日分の`smoking_count`合計を7で割って表示する
- 目標との差は、各ログの`target_daily_cigarette_count_snapshot`の平均と比較する
- 節約額の1日平均は、7日分の`saved_yen`合計を7で割る
- 30日間の節約ペースは、1日平均節約額に30を掛けた金額とする
...
- [ ] 昨日までの直近7日間のログを`current_user`配下から取得する
- [ ] 7日分の記録が揃っているか判定する
- [ ] 平均喫煙本数とsnapshot目標との差を計算する
- [ ] 平均日別節約額と30日間の節約ペースを計算する
- [ ] 目標以内・目標超過それぞれの日本語メッセージを表示する
- [ ] 記録不足時に残り日数を表示する
- [ ] 平均、端数処理、鬼モード、未記録日、他ユーザー除外のテストを追加する
- [ ] Docker経由でテスト、RuboCop、Brakemanを実行する
- [ ] Playwright/MCPでデータ充足時・不足時・スマホ幅・console errorなしを確認する
...
- [ ] 7日分の記録が揃うと、平均喫煙本数と目標との差を確認できる
- [ ] 現在の平均節約額から30日間の節約ペースを確認できる
- [ ] 記録不足時は誤った平均を出さず、必要な残り記録日数が表示される
- [ ] 表示が目安であることと計算対象期間が分かる
