...
- [ ] migration（`user:references`, `smoked_on:date`, `smoking_count default:0`, snapshots **5つ**、index unique）
  - [ ] `target_daily_cigarette_count_snapshot`
  - [ ] `baseline_daily_cigarette_count_snapshot`
  - [ ] `pack_price_snapshot`
  - [ ] `cigarettes_per_pack_snapshot`（箱本数も変更され得るため）
  - [ ] `is_oni_mode_snapshot`（鬼モードON/OFFも当日の計算再現に必要）
- [ ] `UserSmokingLog` に `belongs_to :user`
- [ ] `User` に `has_many :user_smoking_logs, dependent: :destroy`（以降の `current_user.user_smoking_logs` 利用のため）
- [ ] バリデーション（`smoked_on` presence、`smoking_count >= 0`）
